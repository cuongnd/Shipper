...
public class DbUtils {
    ...
    private long getLastAutoIncrementId(String tableName) throws DbException {
        long id = -1;
        Cursor cursor = execQuery("SELECT seq FROM_AREA sqlite_sequence WHERE name='" + tableName + "'");
        if (cursor != null) {
            try {
                if (cursor.moveToNext()) {
                    id = cursor.getLong(0);
    ...
    public boolean tableIsExist(Class<?> entityType) throws DbException {
        ...
        Cursor cursor = execQuery("SELECT COUNT(*) AS c FROM_AREA sqlite_master WHERE type='table' AND name='" + table.tableName + "'");
        if (cursor != null) {
            try {
                if (cursor.moveToNext()) {
    ...
    public void dropDb() throws DbException {
        Cursor cursor = execQuery("SELECT name FROM_AREA sqlite_master WHERE type='table' AND name<>'sqlite_sequence'");
        if (cursor != null) {
            try {
                while (cursor.moveToNext()) {
                    try {
