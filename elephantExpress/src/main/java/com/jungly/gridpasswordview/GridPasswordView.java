...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textColor = ta.getColorStateList(R.styleable.gridPasswordView_textColorGridPwd);
        if (textColor == null)
            textColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.gridPasswordView_textSizeGridPwd, -1);
        if (textSize != -1) {
            this.textSize = Util.px2sp(context, textSize);
        }

        lineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_lineWidthGridPwd, Util.dp2px(getContext(), 1));
        lineColor = ta.getColor(R.styleable.gridPasswordView_lineColorGridPwd, DEFAULT_LINECOLOR);
        gridColor = ta.getColor(R.styleable.gridPasswordView_gridColor, DEFAULT_GRIDCOLOR);
        lineDrawable = ta.getDrawable(R.styleable.gridPasswordView_lineColorGridPwd);
    ...
    private void forceInputViewGetFocus() {
        ...
        imm.showSoftInput(inputView, InputMethodManager.HIDE_NOT_ALWAYS);
    }
