 ...
 * 
 */
public class HuifuCityPicker extends LinearLayout {
	/** 滑动控件 */
...
//	private HashMap<String, List<Cityinfo>> couny_map = new HashMap<String, List<Cityinfo>>();
	...
	private String city_code_string, province_code_string;
	private String province_string, city_string, district_string;

	public HuifuCityPicker(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public HuifuCityPicker(Context context) {
		super(context);
		this.context = context;
		getaddressinfo();
		// TODO Auto-generated constructor stub
	...
	private void getaddressinfo() {
		...
		String area_str = FileUtil.readAssets(context, "json/PnrArea.js");
...
//		length_map = parser.getJSONParserResultArray(area_str, "carLength");
	...
	public static class JSONParser {
		...
		public List<CarInfo> getJsonObject(String json){
			...
			HuifuCity huifuCity = GsonHelper.fromJson(json,HuifuCity.class);
			List<HuifuCity.ProvinceEntity> list =huifuCity.province;
			for (int i=0;i<list.size();i++){
				CarInfo carInfo = new CarInfo();
				carInfo.setId(list.get(i).ProvinceCode);
				carInfo.setCity_name(list.get(i).ProvinceName);
		...
		public HashMap<String, List<CarInfo>> getJsonHashMap(String json){
			...
			for (int i=0;i<list.size();i++){
				...
				for (int j=0;j<list.get(i).Citys.size();j++){
					CarInfo carInfo = new CarInfo();
					carInfo.setId(list.get(i).Citys.get(j).CityCode);
					carInfo.setCity_name(list.get(i).Citys.get(j).CityName);
					cityList.add(carInfo);
					hashMap.put(list.get(i).ProvinceCode,cityList);
				}
			}
			return hashMap;
		}
		...
		cityPicker.setOnSelectListener(new ScrollerNumberPicker.OnSelectListener() {
			...
			public void endSelect(int id, String text) {
				// TODO Auto-generated method stub
					...
					Tracer.e("HuifuPicker",id+" text"+text +" cityList:"+city_code_string);
				if (text.equals("") || text == null)
					return;
				if (temCityIndex != id) {
