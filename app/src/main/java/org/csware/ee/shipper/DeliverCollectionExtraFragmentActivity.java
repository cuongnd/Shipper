 ...
 */
public class DeliverCollectionExtraFragmentActivity extends FragmentActivityBase implements
        InputMethodLinearLayout.OnSizeChangedListenner {

    final static String TAG = "DeliverCollectionExtra";
...
//    LinearLayout boxGoodsAmount;
    ...
    @InjectView(R.id.listViewAuto)
    ScrollViewForListView listViewAuto;
            ...
            public void error(Return json) {
                ...
                if (json.Message != null) {
                    if (json.Result == 2 || json.Message.equals("货主状态错误")) {  //toastSlow("订单号获取失败,请稍候重试");
                        finish();
                        startActivity(new Intent(DeliverCollectionExtraFragmentActivity.this, UserAuthActivity.class));
                    }
                }
            }
        });
...
//                        getResources(), R.drawable.icon_addpic_unfocused));
                ...
                holder.bt.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        bmp.remove(sign);
                        drr.remove(sign);
                        QCloudService.DeleteFile(DeliverCollectionExtraFragmentActivity.this, netdrr.get(sign));
                        netdrr.remove(sign);
                        gridviewInit();
                    }
                });
...
//                    }
                    ...
                    Intent intent = new Intent(DeliverCollectionExtraFragmentActivity.this,
                            PhotoActivity.class);
    ...
    void setLinAddCollection() {
        ...
        Tracer.e(TAG,idMapList.size()+": idMapList");
        autoAdapter.notifyDataSetChanged();
        String price = "";
        if (idMapList.size()>0){
            itemPos = idMapList.size()-1;
            price = idMapList.get(itemPos).get("payee_price_"+itemPos);
        }
        if (!Guard.isNullOrEmpty(price)){
            itemPos++;
            addEmptyData(itemPos);
            if (itemPos > 0) {
    ...
    void setLin_removeView() {
        if (itemPos>0 && idMapList.size()>1){
            idMapList.remove(idMapList.size()-1);
            autoAdapter.notifyDataSetChanged();
            itemPos--;
            if (itemPos == 0) {
     ...
     * 自增控件 listview
     * */
    List<HashMap<String,String>> idMapList = new ArrayList<>();
    CommonAdapter<HashMap<String,String>> autoAdapter;
    void addEmptyData(int pos){
        HashMap<String,String> map = new HashMap<>();
        map.put("payee_area_"+pos,"");
        map.put("payee_address_"+pos,"");
        map.put("payee_name_"+pos,"");
        map.put("payee_mobile_"+pos,"");
        map.put("payee_price_"+pos,"");
        idMapList.add(map);
        if (!Guard.isNull(autoAdapter)){
            autoAdapter.notifyDataSetChanged();
        }
    }
    void autoFindListView(){
        autoAdapter= new CommonAdapter<HashMap<String, String>>(baseActivity, idMapList, R.layout.add_point_item) {
            @Override
            public void convert(org.csware.ee.view.ViewHolder helper, HashMap<String, String> item, final int position) {
                String collAddress = "";
                if (!Guard.isNullOrEmpty(item.get("payee_area_"+position))&& !Guard.isNull(ah)) {
                    String[] names = ah.getName(item.get("payee_area_"+position));
                    collAddress = ParamTool.join(names, ",") + " " + item.get("payee_address_"+position);
                }
                helper.setText(R.id.txtPayeeCollectionAddress,collAddress);
                String name = item.get("payee_name_"+position);
                String mobile = item.get("payee_mobile_"+position);
                helper.setText(R.id.txtCollectName,name+"\t"+ mobile);
//                helper.setText(R.id.txtCollectPhone,mobile);
                String price = "";
                if(!Guard.isNullOrEmpty(item.get("payee_price_"+position))){
                    price = item.get("payee_price_"+position)+"元";
                }
                helper.setText(R.id.txtCollectMoney, price);
                TextView textView =  helper.getView(R.id.txtCollectName);
                if (!Guard.isNullOrEmpty(item.get("payee_price_"+position))) {
                    textView.setHint("");
                    helper.getView(R.id.LinCollectionMoney).setVisibility(View.VISIBLE);
                }else {
                    String collectName = name+"\t"+ mobile;
                    if (Guard.isNullOrEmpty(collectName.trim())){
                        textView.setText("");
                    }else {
                        textView.setText(collectName);
                    }
                    textView.setHint(getString(R.string.deliver_collection_consignee));
                    helper.getView(R.id.LinCollectionMoney).setVisibility(View.GONE);
                }
                //add some point
                LinearLayout layout = helper.getView(R.id.Lin_PayAddIcons);
                if (position<(idMapList.size()-1)) {
                    layout.setVisibility(View.VISIBLE);
                    OrderBidFragmentActivity.AddPoint(layout, baseActivity);
                }else {
                    layout.setVisibility(View.GONE);
                }
                helper.getView(R.id.LinAutoToAddress).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        pos = position;
                        Intent intent = new Intent(DeliverCollectionExtraFragmentActivity.this, DeliverSelectorAddressActivity.class);
                        intent.putExtra("isFrom", false);
                        intent.putExtra("type", 2);
                        startActivityForResult(intent, 101);
                    }
                });
                helper.getView(R.id.LinAutoToInfo).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        pos = position;
                        if (!Guard.isNullOrEmpty(idMapList.get(position).get("payee_area_"+position))) {
                            Intent intent = new Intent(DeliverCollectionExtraFragmentActivity.this, DeliverAddCollectionActivity.class);
        ...
        listViewAuto.setAdapter(autoAdapter);
        addEmptyData(0);
    }
    //存自增控件ID集合
...
//        addView(Lin_AddCollectionItem,infoLin);
    ...
    void setBtnDeliver() {
        if (btnDeliver.getText().toString().equals("下一步")) {
            ...
            if (idMapList.size()>0) {
                int mapSize = idMapList.size()-1;
                String areas = idMapList.get(mapSize).get("payee_area_"+mapSize);
                String consignee = idMapList.get(mapSize).get("payee_price_"+mapSize);
...
//            Tracer.e(TAG,_info.memo+"");
            ...
            if (idMapList.size()>0) {
                for (int i = 0; i < idMapList.size(); i++) {
                    hashMap.putAll(idMapList.get(i));
                }
            }
            Tracer.e(TAG, hashMap.toString());
        ...
        switch (requestCode) {
            ...
            case 101:
                if (resultCode == 111) {
                    ...
                    HashMap<String,String> map = idMapList.get(pos);
                    map.put("payee_area_"+pos,_tmpToCode);
                    map.put("payee_address_"+pos,_tmpDetail);
                    idMapList.set(pos,map);
                    if (!Guard.isNull(autoAdapter)) {
                        autoAdapter.notifyDataSetChanged();
                    }
            ...
            case 102:
                if (resultCode == 111) {
                    if (!Guard.isNull(data)) {
                        ...
                        HashMap<String, String> map = idMapList.get(pos);
                        map.put("payee_name_" + pos, name);
                        map.put("payee_price_" + pos, money);
                        map.put("payee_mobile_" + pos, mobile);
                        idMapList.set(pos, map);
                        if (!Guard.isNull(autoAdapter)) {
                            autoAdapter.notifyDataSetChanged();
                        }
